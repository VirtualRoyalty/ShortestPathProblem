...
class StressTest: 
    ...
    def run_johnsons_floyd_test(self, number_of_iterations, max_nodes, max_edges, heuristic=None):
        ...
        return {'error_graphs' : self.error_graphs, 'johnson' : np.mean(self.working_time['johnson']), 'floyd'  : np.mean(self.working_time['floyd'])}
...
#                                   random.randint(min_coord, max_coord))
    @staticmethod
    def get_path_lengths(G, floyd_shortest_path, dist_floyd, johnson_shortest_path, verbose=True):
